...
class OllamaClient:
    ...
    def chat(
        ...
        """Send a non-streaming chat request to the Ollama model with optional tool support.

        Args:
            messages: List of conversation messages.
            model: Model name to use. Defaults to self.default_model.
            tools: Optional tool schemas exposed to the model.

        Returns:
            ModelResponse containing the response text and any requested tool_calls.

        Raises:
            OllamaConnectionError: If server is unreachable.
            ModelNotFoundError: If requested model is not installed.
    ...
    def stream_chat(
        ...
        """Send a streaming chat request to the Ollama model.
        ...
        Yields:
            Incremental text tokens/chunks as they are generated by the model.
        ...
        logger.info("CHAT START model=%s (messages=%d)", target_model, len(formatted_messages))
